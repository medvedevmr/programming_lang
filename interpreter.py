...
class Interpreter:
    ...
    def compute_bin(self, left, op, right):
        ...
        return tokens.Integer(output) if (left_type == "INT" and right_type == "INT") else tokens.Float(output)
    ...
    def compute_unary(self, operator, operand):
        ...
        return tokens.Integer(output) if (operand_type == "INT") else tokens.Float(output)
    ...
    def interpret(self, tree=None):
        ...
        if isinstance(tree, list):
            if isinstance(tree[0], tokens.Reserved):
                if tree[0].value == "if":
